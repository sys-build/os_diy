...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, struct PIXEL24 *vram, int xsize, int ysize)
{
	struct SHTCTL *ctl;
	int i;
...
void sheet_setbuf(struct SHEET *sht, struct PIXEL24 *buf, int xsize, int ysize, int col_inv)
{
	sht->buf = buf;
	sht->bxsize = xsize;
	sht->bysize = ysize;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1)
{
	...
	struct PIXEL24 *buf, c, *vram = ctl->vram, trans;
	struct SHEET *sht;
	for (h = 0; h <= ctl->top; h++) {
		sht = ctl->sheets[h];
		buf = sht->buf;
        trans = *(struct PIXEL24 *)&sht->col_inv;
